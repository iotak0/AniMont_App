...
import 'package:anime_mont_test/widgets/add_comment_anime.dart';
...
class AnimeCommentsPage extends StatefulWidget {
  ...
  @override
  State<AnimeCommentsPage> createState() => AnimeCommentsPageState();
}

class AnimeCommentsPageState extends State<AnimeCommentsPage> {
  int page = 1;
  ...
  static List<AnimeComments> comments = [];
  ...
  addComment() async {
    setState(() {
      AnimeCommentsPageState.sending = true;
    });

    FocusScope.of(context).requestFocus(FocusNode());
    print("widget.pageId ${widget.pageId}");
    final response = await http.post(Uri.parse(add_anime_comment), body: {
      "comment_anime": '${widget.pageId.toString()}',
      "comment_cont": myController.text.toString(),
      "comment_user": '${widget.myId.toString()}',
      "comment_fair": isFire == false ? "0" : "1",
      "reply_of": "${!reply ? '0' : comments[select].commentId.toString()}"
    }).catchError((e) {
      AnimeCommentsPageState.sending = false;
    ...
    if (body['status'] == "success") {
      ...
      edit = false;
      isFire = false;
      AnimeCommentsPageState.reply = false;
      select = 0;
      refresh();
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: CustSnackBar(
          headLine:body['message'] == "error"? context.localeString('erorr'):context.localeString('you_have_been_banned'),
          erorr: body['message'] == "error"?context.localeString("try_again"):context.localeString("your_account_is_banned"),
  ...
  getComments() async {
    ...
    final response = await http
        .get(Uri.parse(
            '${all_comments_anime}?page=$page&comment_anime=${widget.pageId}&user_id=${widget.myId}'))
        //
        .catchError((erorr0) {
      isLoading = false;
      erorr = true;
    ...
    if (response.statusCode == 200) {
      ...
      for (var e in data) {
        comments.add(AnimeComments(
          e['comment_anime'].toString(),
                                  ...
                                  children: List.generate(
                                    ...
                                    (index) {
                                      ...
                                      getReplies() async {
                                        ...
                                        final response = await http
                                            .get(Uri.parse(
                                                '${all_comments_anime_replies}?page=${comments[index].replyPage}&comment_id=${comments[index].commentId}&user_id=${widget.myId}'))
                                            .catchError((erorr0) {
                                          comments[index].replyLoading = false;
                                      ...
                                      onTap() {
                                        AnimeCommentsPageState.reply = true;
                                        AnimeCommentsPageState.select = index;
...
class CommentCardState extends State<CommentCard> {
  ...
  delete() async {
    ...
    final response = await http.post(
        Uri.parse(
          delete_comments_anime,
        ),
        body: {
          "comment_anime": '${widget.pageId}',
          "comment_id": '${comment.commentId}',
        }).catchError((e) {
      setState(() {
  ...
  edit() async {
    AnimeCommentsPageState.edit = true;
    await showDialog(
        context: context,
        builder: (context) {
          CustomColors customColors = CustomColors(context);
                ...
                children: [
                  Expanded(
                    child: Padding(
                      ...
                      child: TextField(
                        // enabled: !AnimeCommentsPageState.edit,
                        // autofocus: true,
                            ...
                            : () async {
                                ...
                                final response = await http.post(
                                    ...
                                    body: {
                                      ...
                                      "comment_id": '${comment.commentId}',
                                      "fire":
                                          AnimeCommentsPageState.isFire == false
                                              ? "0"
                                              : "1",
                                    }).catchError((e) {
                                  setState(() {
                                    AnimeCommentsPageState.edit = false;
                                    comment.editing = false;
                                  });
                                });
                                final data = jsonDecode(response.body);
  ...
  void initState() {
    ...
    super.initState();
    comment = AnimeCommentsPageState.comments[widget.index];
    setState(() {});
  }
...
class RepliesCard extends StatefulWidget {
  ...
  final UserProfial account;
  final List<AnimeComments> comment;
  final GestureTapCallback onTap;
  final int pageId;
  final int myId;
  final TextEditingController controller;
  @override
  State<RepliesCard> createState() => _RepliesCardState();
}
...
class _ReplyCardState extends State<ReplyCard> {
  like() async {
    ...
    final response = await http.post(
        ...
        body: {
          "user_id": '${widget.account.id}',
          "comment_user": '${widget.comment.account.id}',
  ...
  delete() async {
    ...
    if (data['status'] == "success") {
      setState(() {
        widget.comment.editing = false;
        widget.comment.hasDeleted = true;
      });
      return true;
    } else {
      setState(() {
                          ...
                          GestureDetector(
                            ...
                            child: SizedBox(
                              ...
                              child: LocaleText(
                                ...
                                style: TextStyle(
                                  color:
                                      customColors.primaryColor.withOpacity(.6),
                                  //   fontFamily: 'SFPro',
                                  fontSize: 18,
                                  fontWeight: FontWeight.w500,
                                ),
  ...
  void initState() {
    super.initState();
    avatar = widget.comment.account.avatar.toString().startsWith('http')
        ? widget.comment.account.avatar
        : image + widget.comment.account.avatar;
    if (widget.account.userName != widget.comment.account.userName) {
      CachedNetworkImage.evictFromCache(widget.comment.account.avatar);
    }
  }
  ...
  Widget build(BuildContext context) {
    CustomColors customColors = CustomColors(context);
    return (widget.comment.hasDeleted)
        ? SizedBox()
        : widget.comment.editing
            ? LoadingGif(logo: true,)
            : Padding(
                            ...
                            onTap: () {
                              Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                      builder: (context) => ProfialPage(
                                          id: int.parse(
                                              widget.comment.account.id),
                                          isMyProfile: false,
                                          myId: int.parse(widget.account.id))));
                            },
                            child: Padding(
                              padding: const EdgeInsets.symmetric(
